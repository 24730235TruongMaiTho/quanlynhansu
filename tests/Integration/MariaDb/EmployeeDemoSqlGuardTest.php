...
class EmployeeDemoSqlGuardTest extends MariaDbTestCase
{
    ...
    public function test_guard_failure_stops_mutation_when_client_continues_after_statement_error(): void
    {
        $this->installCanonicalDump();
        $before = [
            'employees' => $this->demoEmployeeCount(),
            'addresses' => $this->demoAddressCount(),
            'departments' => $this->demoDepartmentCount(),
        ];

        $this->createInvalidSessionMarker();
        $result = $this->runDemoSqlContinuingAfterErrors(
            base_path('database/sql/employee/demo/2026_08_21_001_demo_seed.sql'),
        );

        $this->assertSame(1, $result['statements']);
        $this->assertCount(1, $result['errors']);
        $this->assertStringContainsString('DEMO_SEED_GUARD_MARKER_INVALID', $result['errors'][0]);
    ...
    private function createInvalidSessionMarker(): void
    {
        $pdo = $this->pdo();
        $pdo->exec('DROP TEMPORARY TABLE IF EXISTS employee_demo_guard');
        $pdo->exec(
        ...
        $pdo->exec("SET @employee_demo_guard_token = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'");
    }
     ...
     */
    private function runDemoSqlContinuingAfterErrors(string $path): array
    {
        $contents = file_get_contents($path);
        if ($contents === false) {
            throw new RuntimeException('Unable to read demo SQL.');
        ...
        foreach (preg_split('/\R/', $withoutComments) as $line) {
        ...
        if (trim($buffer) !== '') {
            throw new RuntimeException('Demo SQL ended with an incomplete statement.');
        }
    ...
    private function demoAddressCount(): int
    {
        return (int) $this->pdo()->query(
            "SELECT COUNT(*) FROM dia_chi_nhan_vien WHERE ma_nv IN (
                SELECT ma_nv FROM nhan_vien
    ...
    private function demoDepartmentCount(): int
    {
        return (int) $this->pdo()->query(
            "SELECT COUNT(*) FROM phong_ban WHERE ten_pb LIKE '[DEMO-2026-08-21]%'",
        )->fetchColumn();
    }
}

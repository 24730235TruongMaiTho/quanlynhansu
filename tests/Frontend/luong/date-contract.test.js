...
test('coefficient date fields use native ISO date controls', () => {
    for (const id of ['coefficient-from-date', 'coefficient-to-date']) {
        ...
        assert.doesNotMatch(field, /placeholder="dd\/mm\/yyyy"/);
        assert.doesNotMatch(field, /inputmode="numeric"/);
        assert.doesNotMatch(field, /maxlength="10"/);
...
test('coefficient and salary date surfaces use shared strict formatting', () => {
    ...
    assert.match(coefficientJs, /canonicalServerDate/);
    ...
    assert.doesNotMatch(coefficientJs, /formatDisplayDate/);
    assert.doesNotMatch(coefficientJs, /dd\/mm\/yyyy/);
